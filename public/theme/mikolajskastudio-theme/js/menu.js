...
function findIndexToHide(){ //find menu items to hide under "more" button
    var menuwidth = $('.menu').width(),
        limit = $(window).width() - $('.logo-text').width() - ($('#more-navi').width()) - 200;
        ...
        $(menuitems).each(function(){
            ...
            if(index > num){
                var link = $(this).find('a'),
                    ...
                    text = $(link).html(),
                    result;
                result = '<a href="'+href+'" title="'+title+'"><li class="menu-item">'+text+'</li></a>';
                $('.more-navi-items').append(result);
                $(this).hide();
            }
        });
...
function organizeMenu(){ //execute menu items hiding etc.
    ...
    hideMenuItems(index);
    if($('.more-navi-items').find('li').length < 1){
        $('#more-navi').hide()
    }else{
        $('#more-navi').show()
    }
...
$(document).keyup(function(e) { //collapsing search module by tapping/pressing Esc
    if (e.keyCode == 27 && $('.more-navi-items').hasClass('isvisible')) {
            $('.more-navi-items').slideToggle('fast',function(){$('.more-navi-items').removeClass('isvisible')});
    }
});
